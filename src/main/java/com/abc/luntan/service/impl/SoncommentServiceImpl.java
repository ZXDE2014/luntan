...
import com.abc.luntan.mapper.SoncommentMapper;
...
import static com.abc.luntan.utils.RedisConstants.SON_COMMENT_LIKED_KEY;

/**
 *
 */
@Service
public class SoncommentServiceImpl extends ServiceImpl<SoncommentMapper, Soncomment>
    implements SoncommentService {
    ...
    @Override
    public CommonResult querySonComment(Integer current, Integer firstCommentId) {
        ...
        return CommonResult.success(sonCommentIPage);
    }

    @Override
    public CommonResult likeSonComment(Long id) {
        //1获取登录用户
    ...
    @Override
    @Transactional
    public CommonResult createSonComment(SonCommentDTO soncommentDTO) {
        ...
        if (!foundAllSensitive.isEmpty()) {
            return CommonResult.fail("评论内容中含有以下违禁词 " + foundAllSensitive.toString() + " ,请修改后发布");
        }
    ...
    @Override
    public CommonResult detailSonComment(Long id) {
        Soncomment soncomment = getById(id);
        if (soncomment == null) {
            return CommonResult.fail("没有这条评论");
        }
        ...
        return CommonResult.success(soncomment);
    }
